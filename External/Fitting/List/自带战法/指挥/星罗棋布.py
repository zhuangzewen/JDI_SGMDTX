...
class 星罗棋布_阵型强化_soul(BaseSkillSoul):
    ...
    def response(self, status = SoulResponseTime.无响应阶段, battleField=None, hero: Hero = None, sourceSoul: Soul = None):
        ...
        for 己方阵型强化soul in 对己方阵型强化SOUL生效(self.target, battleField):
            ...
            for exist_soul in self.soul持有列表:
                if exist_soul.sourceSoul == self and exist_soul.target == targetHero and exist_soul.effectType == 己方阵型强化soul.effectType and exist_soul.skill == self.skill:
                    存在未强化的阵型SOUL = False
                    break
            if 存在未强化的阵型SOUL:
...
class 星罗棋布_谋略减伤_soul(BaseSkillSoul):
    ...
    def response(self, status = SoulResponseTime.无响应阶段, battleField=None, hero: Hero = None, sourceSoul: Soul = None):
        ...
        for 目标武将 in valueList:
            ...
            谋略减伤soul = Soul(target=目标武将, 
                            ...
                            effectValue= - reduce_value)
            谋略减伤soul.deploy_initial()
            self.soul持有列表.append(谋略减伤soul)

class 星罗棋布_额外效果_soul(BaseSkillSoul):
    ...
    def response(self, status = SoulResponseTime.无响应阶段, battleField=None, hero: Hero = None, sourceSoul: Soul = None):
        ...
        Log().battle_L2('[{}]执行来自【{}】的[星罗棋布-额外效果]效果'.format(self.target.get_武将名称().value, self.skill.get_战法名称().value))
        
        if msg_判断己方前排武将数量(self.target, battleField) == 1:
            Log().battle_L2('[{}]执行来自【{}】的[星罗棋布-单前排阵型]效果'.format(self.target.get_武将名称().value, self.skill.get_战法名称().value))
            ...
            固定受击率soul = Soul(target=frontLineHero,
                            ...
                            sourceType=SoulSourceType.武将战法, 
                            skill=self.skill, 
                            effectType=SoulEffectType.固定受击率, 
                            effectValue=0.85)
            固定受击率soul.deploy_initial()
            self.soul持有列表.append(固定受击率soul)
            ...
            单前排减伤soul = Soul(target=frontLineHero, 
                        ...
                        effectValue= - self.skill.星罗棋布_单前排_受到伤害降低系数())
            ...
            对前排增伤soul = Soul(target=lowest_ts_hero, 
                            ...
                            sourceType=SoulSourceType.武将战法, 
                            skill=self.skill, 
                            effectType=SoulEffectType.对前排造成伤害提升, 
                            effectValue= self.skill.星罗棋布_双前排_对前排造成伤害提升系数())
            对前排增伤soul.deploy_initial()
            self.soul持有列表.append(对前排增伤soul)

            星罗棋布双前排额外效果soul = 星罗棋布_额外效果_双前排阵型(
                                                target=lowest_ts_hero,
                                                initiator=self.skill.get_持有者(),
                                                sourceType=SoulSourceType.武将战法,
                                                skill=self.skill,
                                                damage=Damage(skillEffectName="星罗棋布-双前排阵型-借刀"))
            ...
            Log().battle_L2('[{}]的[星罗棋布-双前排阵型]效果已施加'.format(lowest_ts_hero.get_武将名称().value))

            self.damage = Damage(skillEffectName="星罗棋布-双前排阵型")

        elif msg_判断己方前排武将数量(self.target, battleField) == 3:
            Log().battle_L2('[{}]执行来自【{}】的[星罗棋布-三前排阵型]效果'.format(self.target.get_武将名称().value, self.skill.get_战法名称().value))
            ...
            Log().battle_L2('[{}]的[星罗棋布-三前排阵型]效果已施加'.format(self.target.get_武将名称().value))
...
class 星罗棋布_额外效果_双前排阵型(BaseSkillSoul):
    ...
    def response(self, status=SoulResponseTime.无响应阶段, battleField=None, hero: Hero = None, sourceSoul: Soul = None):
        ...
        Log().battle_L1('[{}]执行来自【{}】的[星罗棋布-双前排阵型]效果'.format(self.target.get_武将名称().value, self.skill.get_战法名称().value))
        ...
        for _ in range(attack_times):
            ...
            if len(attacked_heroes) == 0:
                Log().battle_L1('[{}]没有可攻击的敌方武将'.format(attaHero_name.value))
                break
...
class 星罗棋布_skill(BaseSkill):
    ...
    def fill_init_soul(self):
        ...
        星罗棋布阵型强化soul = 星罗棋布_阵型强化_soul(
            ...
            skill=self,
            damage=Damage(skillEffectName="星罗棋布-阵型强化")
        )
        ...
        星罗棋布谋略减伤soul = 星罗棋布_谋略减伤_soul(
            ...
            initiator=self.get_持有者(),
            skill=self,
            damage=Damage(skillEffectName="星罗棋布-谋略减伤")
        )
